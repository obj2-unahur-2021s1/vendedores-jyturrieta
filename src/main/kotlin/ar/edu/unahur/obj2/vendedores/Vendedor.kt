...
class Viajante(val provinciasHabilitadas: List<Provincia>) : Vendedor() {
  ...
  fun poblacionDeProvinciasHabilitadas() = provinciasHabilitadas.sumBy{c -> c.poblacion}
}
...
class ComercioCorresponsal(val ciudades: List<Ciudad>) : Vendedor() {
  ...
  fun cantidadDeCiudades() = ciudades.size
  fun cantidadDeProvincias() = ciudades.map{c->c.provincia}.toSet().size

  override fun esInfluyente() = this.cantidadDeCiudades() >= 5 || this.cantidadDeProvincias() >= 3
